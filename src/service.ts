...
export function createTokenomicsService() {
  ...
  const service = {
    ...
    start(ctx: OpenClawPluginServiceContext) {
      ...
      // Prefer the dedicated content-free model-usage stream (OpenClaw
      // ctx.modelUsage, openclaw#97892): it is pre-filtered to model.usage and
      // host-gated, so no forgeable-emitter filtering is needed. Fall back to the
      // trusted internal diagnostics bus on hosts that predate that surface.
      const modelUsage = (
        ctx as {
          modelUsage?: { onEvent(listener: (event: ModelUsageEvent) => void): () => void };
        }
      ).modelUsage;
      if (modelUsage) {
